# src/vision.py
# YOLOv8 tree detection + green-density fallback.
# Camera must be FORWARD-FACING, tilted ~30° down from horizontal.
# RowVision runs in its own daemon thread; .read() is always non-blocking.
...
import config as cfg   # imported so VisionResult properties read live cfg values
...
class TreeDetection:
    ...
    width: int
    height: int
    confidence: float    # 0.0–1.0  (1.0 for contour fallback)
    distance_hint: float = 0.0   # estimated metres from bbox height


@dataclass
class VisionResult:
    green_density: float = 0.0
    trees: list = field(default_factory=list)   # list[TreeDetection]
...
class RowVision:
    """
    Runs YOLOv8 (or contour fallback) in a daemon thread.
    Thread-safe: .read() → VisionResult | None, .age() → float.
    """
    ...
    # COCO80: class 63 = "potted plant" (only correct plant-proxy in COCO)
    TREE_CLASS_IDS_COCO = {63}
    ...
    def __init__(self, model_path: str = None,
        ...
        if _YOLO_OK and model_path:
            try:
                ...
                log.warning(f"YOLO load failed ({e}) — contour fallback")
    ...
    def _loop(self):
        ...
        if not self._cap.isOpened():
            log.error("Camera unavailable — vision thread inactive")
            return
    ...
    def _detect_yolo(self, frame: np.ndarray) -> list:
        ...
        try:
            ...
            log.debug(f"YOLO infer error: {e}")
            return detections
        for box in res.boxes:
            ...
            dist   = (2.5 * 600) / max(bh, 1)   # pinhole approx
    ...
    def _pick_best(self, trees: list) -> TreeDetection:
        ...
        def score(t: TreeDetection) -> float:
